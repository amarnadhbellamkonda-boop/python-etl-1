...
def load_payments(cursor, update_timestamp):
    table = "Payments"
    columns_str = os.getenv("PAYMENTS_COLUMNS")
    db_link = os.getenv("DB_LINK")
    
    if not columns_str:
        raise ValueError("PAYMENTS_COLUMNS not set in .env")
...
def export_payments(update_timestamp = None):
    table = "Payments"
    ...
    try:
        ...
        filename = load_payments(cursor, update_timestamp)
