...
def extract_table():
    ...
    df = pd.read_sql(f"SELECT CUSTOMERNUMBER, CUSTOMERNAME,CONTACTLASTNAME,CONTACTFIRSTNAME,PHONE,ADDRESSLINE1,ADDRESSLINE2,CITY,STATE,POSTALCODE,COUNTRY,SALESREPEMPLOYEENUMBER,CREDITLIMIT FROM {schema}.{table_name}", conn)
    conn.close()
